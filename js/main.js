...
document.addEventListener("DOMContentLoaded", function () {
  ...
  const addHighlightTool = function () {
    ...
    const highlightShrinkEle =
      isHighlightShrink !== undefined
        ? '<i class="beatricefont beatrice-icon-angle-down expand ${highlightShrinkClass}"></i>'
        : "";
    const highlightCopyEle = highlightCopy
      ? '<div class="copy-notice"></div><i class="beatricefont beatrice-icon-paste copy-button"></i>'
      : "";
    ...
    const createEle = (lang, item, service) => {
      ...
      if (highlightHeightLimit && item.offsetHeight > highlightHeightLimit + 30) {
        ...
        ele.innerHTML = '<i class="beatricefont beatrice-icon-angle-double-down"></i>';
  ...
  const rightSideFn = {
    readmode: () => {
      ...
      newEle.type = "button";
      newEle.className = "beatricefont beatrice-icon-xmark exit-readmode";
  ...
  const listenNavMusicPause = function () {
    const timer = setInterval(() => {
      if (navMusicEl && navMusicEl.querySelector("#nav-music meting-js").aplayer) {
        clearInterval(timer);
        let msgPlay = '<i class="beatricefont beatrice-icon-play"></i><span>播放音乐</span>';
        let msgPause = '<i class="beatricefont beatrice-icon-pause"></i><span>暂停音乐</span>';
